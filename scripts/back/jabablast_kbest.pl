...
The maximum number of unique BLAST hits to use for creating edges. This
is the maximum number of edges that will be drawn. 
...
my $BlastFormat;               # Foramt of the blast file
                               # ie m = 0,8, or 9
my $KMax;                      # The max number of edges to return
                               # This is the "K Best Blast Hits"
                               # Algorithm
my $KCount = 0;                 # Counter for the K Best BLAST
...
#    my $TooMany = 100;
    my $HitNum = 0;
    my $TotNumEdges = 0;   # Total number of edges
    my $NumEdges = 0;      # Number of edges drawn out from an 
                           # individual Qry

    # The query ID is used to increment the number of edges returned
    # for each query.
    # Concatenated IDs are used to return only a single edge for each
    # Query Subject pair instead of each HSP
    # These could be used in combination to create at DrawEdge boolean
    # value
    my $CurX = "";     # Current query ID
    my $PreX = "";     # Previous query ID
    my $CurCatID = ""; # Current concat ID
    my $PreCatID = ""; # Previous concat ID
    my $DrawEdge = 1;  # Boolean, Set to false (0) when  $NumEdges >> KMax
...
#	    $YCrd = $tmphitid[0];
		...
		# Determine if we have hit the max number of
		# edges to be drawn in those cases where 
		# we are using KMax
		#
		if ($KMax){
		    if ($NumEdges > $KMax){
		...
		#print "X: $CurX\tE: $NumEdges\tY:$YCrd\n";
		    ...
		    $DrawEdge == '1')
		{
		    ...
		    if ($XCrd < $YCrd)
		    {
			# Temp print while I work with these
			print "\tX: $CurX\tE: $NumEdges\tY:$YCrd\n";
	    ...
	    # Set the Pre Values for the next round
	    $PreCatID = $CurCatID;
	    $PreX = $CurX;
...
#   no longer uses the BioPERL BLAST parser. This allowed for a
#   HUGE increase in speed.
# - This works for the SanMiguel database
#
# 05/21/2007
# - Work on adding k-best-hits variable to the ParseTabBLAST  
#   subfunction.
# - I think that the k-best-hit alg will need to be implementd
#
