...
#-----------------------------------------------------------+
#                                                           |
# batch_dottup.pl - Make self-self dotplots in batch mode   |
#                                                           |
#-----------------------------------------------------------+
#                                                           |
...
#                                                           |
# DESCRIPTION:                                              |
#  Run self comparision dotplots for all sequences in the   |
#  intput directory using the EMBOXX dottup program. This   |
#  is useful when looking for strings of repeats.           |
#                                                           |
# USAGE:                                                    |
#  batch_dottup.pl -i indir -o outdir                       |
#                                                           |
# VERSION: $Rev$                                            |
#                                                           |
# LICENSE:                                                  |
...
# DOTTUP OPTIONS
my $word_size = 10;
my $graph_type = "png";        # type of graph file output
...
my $ok = GetOptions(# REQUIRED OPTIONS
		    "i|indir=s"     => \$indir,
                    "o|outdir=s"    => \$outdir,
                    "w|wordsize=s"  => \$word_size,
		    "g|graph=s"     => \$graph_type,
		    # ADDITIONAL OPTIONS
		    "q|quiet"       => \$quiet,
		    "verbose"       => \$verbose,
...
#dottup huck_912_ltr.fasta huck_912_ltr.fasta -auto -wordsize 10 -graph png -goutfile huck_912.png
    my $du_cmd = "dottup ".
	$indir.$ind_file." ".$indir.$ind_file.
	" -auto".
	" -wordsize ".$word_size.
	" -graph ".$graph_type.
	" -gtitle ".$name_root.
	" -goutfile ".$outdir.$name_root;
