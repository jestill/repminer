...
# UPDATED: 09/14/2006                                       |
#                                                           |
# DESCRIPTION OF CLUST ANALYSIS:                            |
#  09/14/2006                                               |
#  Modified from existing PAN analysis to work with a       |
#  single FASTA file and just BLAST the sequences against   |
#  the databases of Rep Elements and parse the output to    |
#  a single format.                                         |
#                                                           |
# DESCRIPTION OF PAN ANALYSIS:                              |
...
#                                                           |
# NOTES OF JOANN:                                           |
#  -This run uses an e-value of 1xe-6                       |
#                                                           |
#-----------------------------------------------------------+
...
#-----------------------------+
my $MaxE = '0.000001';        # Max E value for repblast
...
my $SourceDbName;
my @Name;
my $InSeqNum = "0";           # Number of individual sequences
my @SeqNames;                 # Array to holde nam of all seq clusters

#-----------------------------+
# SPLIT THE CLUST FILE INTO   |
# SEPARATE FASTA FILES TO MAKE|
# PARSING THE OUTPUT A BIT    |
# EASIER                      |
#-----------------------------+
...
print METAOUT "<TITLE>JABA BLAST METADATA</TITLE>".
    "</HEAD>\n<BODY>\n";
print METAOUT "<H1>".$Species." Cluster Analysis</H1>\n";
print METAOUT "<HR>\n";
...
# FASTA FILE, CREATE DIRS FOR |
# EACH OF THE FILES THEN      |
# BLAST AGAINST THE REPEAT    |
# DATABASE                    |
#-----------------------------+
while (my $inseq = $seq_in->next_seq)
{
    #$BaseDir = "/home/jestill/projects/asgr/200609/";
    $InSeqNum++;
    print $InSeqNum."\n";
    
    my $IndSeqName = $inseq->primary_id() || "ERROR";
    print "\t".$IndSeqName."\n";
    
    #-----------------------------+
    # MAKE A DIR TO HOLD THE      |
    # INDIVIDUAL FASTA FILE AND   |
    # RELATED ANALYSIS            |
    #-----------------------------+
    my $OutDir = $BaseDir.$IndSeqName."/";
    mkdir $OutDir, 0777 unless (-e $OutDir); 

    #-----------------------------+
    # WRITE SEQUENCE FILE TO THE  |
    # OUTPUT DIR                  |
    #-----------------------------+
    my $SeqOutFile = $OutDir.$IndSeqName.".fasta";
    my $seq_out = Bio::SeqIO->new('-file' => ">$SeqOutFile",
				  '-format' => $informat);
    $seq_out->write_seq($inseq);


    #-----------------------------+
    # PRINT THE NAME OF THE FASTA |
    # FILE THE HTML META FILE     |
    #-----------------------------+
    print METAOUT "<H3>$IndSeqName</H3>\n";


    #-----------------------------+
    # RUN THE BASIC ANALYSIS OF   |
    # THE ASSEMBLY                |
    #-----------------------------+
    ...
    &AsmAnalysis( $ClustWorkDir , $IndSeqName );
...
# TEMP EXIT WHILE I WORK OUT
# LOADING SEQUENCES FROM FASTA FILE INTO
# INDIVIDUAL FOLDERS AND FILES FOR EACH 
...
{
#-----------------------------------------------------------+
# SUBFUNCTION TO ANALYZE INDIVIDUAL FASTA FILES
#-----------------------------------------------------------+
