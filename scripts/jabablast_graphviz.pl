...
use GraphViz;                  # GraphViz graph drawing program
...
my %Options;
getopts('a:b:c:d:e:f:hi:l:m:n:o:r:s:u:x:y:CE:GL:S:MQZ:', \%Options);
...
#&LoadAllByAll;
# 05/16/2007
# Comment out to work on GraphViz format
#
#if ($BlastFormat == '8')
#{
#    print "Parsing tab delim BLAST file.\n";
...
#-----------------------------+
# TEST OF USING GraphViz      |
#-----------------------------+
if ($RunGraphviz)
{
    if ($BlastFormat == '8')
    {
	&TabBlast2GraphViz($AllByAll);
    }
}
...
#	    $YCrd = $tmphitid[0];
		...
		if ($XCrd > $YCrd)
		{
		    ...
		    print BITOUT $XCrd." (bot) ".$YCrd." = ".$BitScore."\n";
...
#  - $BitScore -> Bit Score of hit
    ...
    # Need to add code to make directe be set by th
    # user choice of graph direction
    my $g = GraphViz->new(
			  #layout => 'neato', 
			  layout => 'fdp', 
			  ratio => 'compress',
    ...
    # First attempt to do this will first load all possible 
    # nodes then will add the edges between the nodes


    #-----------------------------+
    # LOAD ALL POSSIBLE NODES     |
    #-----------------------------+
    print "Adding nodes to GraphViz object\n";
    $HitNum = 0;
    my $PrevX = "NULL";
    my $NumNodes = 0;
    while (<BLASTIN>)
...
#	    } 
    ...
    # ADD EDGES                   |
    #-----------------------------+
    print "Adding edges to the GraphViz object\n";
    $HitNum = 0;
    # The CatIDs below were added to allow to ignore
    # mulitple HSPs for the same nodes
...
#	    } 
	    ...
	    #-----------------------------+
	    $GraphDir = 0;
	    # THE DEFAULT GRAPH DIRECTION IS == 0
	    if ($GraphDir == '0')
	    {
		#-----------------------------+
		# UNDIRECTED x < y            |
		...
		if ($XCrd < $YCrd)
		{
			...
			$g -> add_edge( $XCrd => $YCrd, weight => $PID );
			#$g -> add_edge( $XCrd => $YCrd, 
			#		label => $BitScore );
		    }
		    $PreCatID = $CurCatID;
		    # Possible things to add here include
		    # weight [integer]
		    # label [string]
    ...
    # To print the dot file with no layout do as_cannon
    # print $g->as_cannon;

    # The layed out format file would be
    # print $g->as_text;
    ...
    #-----------------------------+
    # OPEN THE GRAPH AS A PNG FILE|
    #-----------------------------+
    print "Nodes:\t$NumNodes\n";
    print "Edges:\t$NumEdges\n";
    my $CurTime = time;
    my $LoadTime = $CurTime - $StartTime;
    print "DefTime:$LoadTime seconds\t";

#    print "Sending GraphViz object to a PNG file.\n";
#    my $png_data = $g->as_png;
#    my $EndTime = time;
#    my $FullTime = $EndTime - $StartTime;
#    print "DrawTime:\t$FullTime\n";
#    open (DISPLAY,"| display -") || die;
#    binmode DISPLAY;
#    print DISPLAY $png_data;
#    close DISPLAY;

    print "Writing the canonical text file\n";
    my $CanPath = "/home/jestill/projects/RepMiner/GV_CanTest.txt";
    open (CANOUT, ">".$CanPath) ||
	die "Can not open the CanPath:\n$CanPath\n";
    print CANOUT $g->as_canon;
    my $png_data 
...
sub PrintHelp
{
	...
	"    default = Network\n".
	" -G Produce GraphViz output [boolean]]\n".
...
# - Modifying the BLAST parser to try to just get a single 
#   edge between any two nodes. This should either ignore 
#   multiple hits to merge all HSPs into a single hit.
# 
# 
# WANTED
# - Assignement and association of networks to sets of PANs from
#   within in the code. May use SQL if possible. Otherwise may
#   want to consider external network evaluation programs.
